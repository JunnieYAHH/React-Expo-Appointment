...
const AcceptedAppointment = () => {
    ...
    useEffect(() => {
        const getAcceptedAppointments = async () => {
            try {
                ...
                const result = await axios.get(`${baseURL}/appointments/get-accepted-appointments`, config)
                // console.log(result.data.acceptedAppointment.user)
                setAppointments(result.data.acceptedAppointment)
    ...
    const handleCompleteAppointment = async (id) => {
        try {
            ...
            const response = await axios.put(`${baseURL}/appointments/complete-appointment/${id}`, config);

            setAppointments(prevAppointments => prevAppointments.filter(appointment => appointment._id !== id));

            Alert.alert('The Appointment Was Now Completed');
        } catch (error) {
            Alert.alert('Status Cannot Be Completed');
            console.log('Error In Completing Status', error.message);
        }
    }
    ...
    return (
        <ScrollView>
            <View style={{ padding: 5, marginLeft: 2 }}>
                <Text style={{ textAlign: 'center', fontSize: 22 }}>Accepted Appointments</Text>
                                ...
                                <View style={{ padding: 15, borderWidth: 4 }}>
                                    <View style={{ padding: 15, borderWidth: 1, borderRadius: 5, alignItems: 'center' }} >
                                        ...
                                        <View style={{ flexDirection: 'row' }}>
                                            <TouchableOpacity
                                                onPress={() => handleCompleteAppointment(appointment._id)}
